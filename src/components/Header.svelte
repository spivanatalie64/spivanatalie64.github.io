<script>
  let { toggleTheme, darkMode, onselect } = $props();
  ...
  function toggleSidebar() {
    ...
    if (sidebarOpen && githubRepos.length === 0 && gitlabRepos.length === 0) {
      fetchRepos();
    }
  }
  ...
  async function fetchRepos() {
    ...
    try {
      ...
      const gh = [];
      if (results[0].status === 'fulfilled') gh.push(...results[0].value);
      if (results[1].status === 'fulfilled') gh.push(...results[1].value);
      githubRepos = gh.filter((r, i, a) => a.findIndex(x => x.name === r.name) === i);

      const gl = [];
      if (results[2].status === 'fulfilled') gl.push(...results[2].value);
      if (results[3].status === 'fulfilled') gl.push(...results[3].value);
      gitlabRepos = gl.filter((r, i, a) => a.findIndex(x => x.name === r.name) === i);
    } catch {}
    loading = false;
  }

  function selectRepo(repo, source) {
    closeSidebar();
    if (onselect) {
      onselect(new CustomEvent('select', {
        detail: {
          ...
          updated: repo.updated_at || repo.last_activity_at || null,
...
{#if sidebarOpen}
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-body">
      {#if loading}
        <div class="sidebar-loading"><i class="fas fa-spinner fa-pulse"></i> Loading...</div>
      {:else}
...
<style>
  ...
  .sidebar-loading i { margin-right: 6px; }
